...
public class QuestionController {
    ...
    @GetMapping("filter")
    public ResponseEntity<List<Question>> filter(@RequestParam String topic,@RequestParam String difficulty){
        return ResponseEntity.ok(questionService.filter(topic,difficulty));
    }

    @GetMapping("random")
    public ResponseEntity<List<Question>> getRandomQuestions(@RequestParam String count){
